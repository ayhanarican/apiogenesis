...
const defaultErrorMessages = require('./messages.json')
...
const findFirstString = (...vals) => vals.filter(_.isString).shift()

const interpolateMessageWithArgs = (message = '', args = []) =>
  message.replace(/{ARGS\[(\d+)\]}/gi, (match, submatch) => args[submatch] || '')

const createValidator = function createValidator(name, fn, args, passIfEmpty) {
...
const validate = function validate(options) {
  ...
  const validator = createValidator(options.validator, validatorFn, args, passIfEmpty)
...
validate.exists = function (name) {
  return validatorjs[name] || customValidators[name] ? true : false
}
